...
- **PR labeling and label gate**: [.github/workflows/labeler.yml](.github/workflows/labeler.yml) auto-applies path-based labels via `actions/labeler` and runs a label-checker that requires every PR to carry at least one of a fixed label set (`breaking`, `security`, `feature`, `bug`, `refactor`, `upgrade`, `docs`, `internal`). Uses `pull_request_target`; same constraint applies (no PR-code checkout). Permissions: `contents: read`, `pull-requests: write` for labeler, `pull-requests: read` for the checker.
