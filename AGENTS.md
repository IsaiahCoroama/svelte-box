...
Instructions for AI coding agents in this repo. Read before changes.

## What this project is

`svelte-box` = tiny reactive container library for Svelte 5. Wraps `$state` so value pass across function, class, component boundaries without losing reactivity. Published to npm as `@coroama/svelte-box`, built via SvelteKit packager. Repo and project keep name `svelte-box`; only npm import path scoped.
...
- **Commit messages**: [Conventional Commits](https://www.conventionalcommits.org/en/v1.0.0/) format. Types: `feat`, `fix`, `refactor`, `perf`, `docs`, `test`, `chore`, `build`, `ci`, `security`, `revert`. Append `!` for breaking, include `BREAKING CHANGE:` footer. Full rules in [CONTRIBUTING.md](CONTRIBUTING.md#commit-messages).
- **Comments only when why non-obvious**. Comment that restate code = noise. README and tests carry explanation.
...
**Node version.** SvelteKit 2.57+ need Node 20.6 or newer (uses `node:util.styleText`). If `bun run check` or `bun run test` fail with `SyntaxError: ... 'styleText'`, active Node binary too old. Use `nvm use 24` (matches publish workflow), or prepend Node 20.6+ install to `PATH` for session.

**Playwright.** Tests use `@vitest/browser-playwright` with Chromium. If Chromium missing, run `bunx playwright install chromium`. Cache lives under `~/.cache/ms-playwright` on Linux, `~/Library/Caches/ms-playwright` on macOS, `%LOCALAPPDATA%\ms-playwright` on Windows.
...
All library source under [src/lib/](src/lib/). Runtime split group related concerns into subdirectories under `core/`. Hand-written `.d.ts` siblings shadow JS files at package-time, see "Why hand-written `.d.ts`" below.
...
- [src/lib/core/core.svelte.js](src/lib/core/core.svelte.js) and [`.d.ts`](src/lib/core/core.svelte.d.ts) : four reactive-cell roots (`CoreBox`, `MutCoreBox`, `RawCoreBox`, `RawMutCoreBox`), `BoxCell<T>` and `AnyBox<T>` types, `isBox()` runtime guard. Two roots: deep `$state` (`CoreBox`/`MutCoreBox`) and `$state.raw` (`RawCoreBox`/`RawMutCoreBox`). `Mut*` siblings add public `value` setters via `[VALUE_SET]` symbol seam.
- [src/lib/core/mixins.js](src/lib/core/mixins.js) and [`.d.ts`](src/lib/core/mixins.d.ts) : every mixin factory and its type-only surface class. Includes `BoxGuardsMixin`, `BoxAccessorMixin` (composes `BoxGetterMixin`+`BoxSetterMixin`+`BoxDeleterMixin`), `BoxCommonMixin` (composes `BoxSerializableMixin`+`BoxFreezableMixin`+`BoxCloneableMixin`), and variadic `BoxMixer` helper. `.d.ts` declares each mixin surface (`BoxGuards`, `BoxAccessor<T>`, `BoxSerializable<T>`, `BoxFreezable`, `BoxCloneable<T>`, accessor primitives) plus `BoxConstructor` and `BoxMixin<B, M>`.
...
- [src/lib/core/proxy/box.js](src/lib/core/proxy/box.js) and [`.d.ts`](src/lib/core/proxy/box.d.ts) : `Box<T>` (proxy-backed subclass of `BaseBox`), `box()` factory, `Boxed<T>` type. Constructor returns `buildBoxProxy(this, opts)`.
- [src/lib/core/proxy/const.js](src/lib/core/proxy/const.js) and [`.d.ts`](src/lib/core/proxy/const.d.ts) : `ConstBox<T>` (read-only proxy variant extending `ConstFastBox`), `constbox()` factory, `ConstBoxed<T>` type.
...
- [src/lib/core/collections/map.js](src/lib/core/collections/map.js) and [`.d.ts`](src/lib/core/collections/map.d.ts) : `boxedMap`, `fastBoxedMap`, `constBoxedMap`, `constFastBoxedMap` factories plus four `*BoxedMap` types. Imports `SvelteMap` from `svelte/reactivity`.
- [src/lib/core/collections/set.js](src/lib/core/collections/set.js) and [`.d.ts`](src/lib/core/collections/set.d.ts) : `boxedSet`, `fastBoxedSet`, `constBoxedSet`, `constFastBoxedSet` factories plus four `*BoxedSet` types. Imports `SvelteSet` from `svelte/reactivity`.
...
- [tests/box.svelte.test.ts](tests/box.svelte.test.ts), [tests/fastbox.svelte.test.ts](tests/fastbox.svelte.test.ts), [tests/const.svelte.test.ts](tests/const.svelte.test.ts), [tests/lazy.svelte.test.ts](tests/lazy.svelte.test.ts) : browser-mode vitest files covering Box, FastBox, ConstBox, LazyBox.
...
- [benchmarking/box.svelte.bench.ts](benchmarking/box.svelte.bench.ts) : benchmark suite for Box and FastBox. Most describe-blocks = three-way Baseline-vs-Box-vs-FastBox comparison. Two `type guards (...)` groups omit Baseline because helpers Box-specific; rest (incl. forwarded-method groups) keep Baseline column.
...
Split between `proxy/`, `fast/`, `collections/` **intentional for tree-shaking**. Importing only `Box` not pull in `SvelteMap`/`SvelteSet`. Splitting `collections/` into `map.js` and `set.js` mean importing only `boxedMap` not pull in `SvelteSet`. Do not fold back into single module.

Split between `base.svelte.js`, `fast/box.js`, `proxy/box.js` also intentional. `BaseBox` = shared parent assembled from mixin chain. `FastBox` and `Box` only differ in constructors and chosen `Const*` variant. Mutating helpers live in `mixins.js` so new behavior added there once, picked up by every class on chain. Do not duplicate methods across subclasses.

Mixin factories in `mixins.js` (`BoxGuardsMixin`, `BoxAccessorMixin`, `BoxSerializableMixin`, `BoxFreezableMixin`, `BoxCloneableMixin`, plus composites `BoxAccessorMixin` and `BoxCommonMixin`) = seam new helper categories added to. `ConstFastBox` reuse `BoxGuardsMixin`, `BoxGetterMixin`, `BoxCommonMixin` to expose read-only side of API without inheriting mutating accessors. d.ts side declares each mixin as type-only `class` (`BoxGuards`, `BoxGetter`, etc.); never instantiated at runtime, only used as `M` type argument to `BoxMixin<B, M>`. Variadic stacking also available through `BoxMixer(Base, ...factories)`, which `BaseBox` and `ConstFastBox` use; see `mixins.d.ts` JSDoc for per-step constraint trade-off.

### Why hand-written `.d.ts`

Project intentionally keep type declarations in separate `.d.ts` files instead of JSDoc-generated. `@sveltejs/package` detects sibling `.d.ts` files and use them instead of regenerating from JSDoc. Build output confirms with lines like `Using $lib/core/proxy/box.d.ts instead of generated .d.ts file`. Do not add full JSDoc type annotations to JS files. Keep types in `.d.ts` siblings.
...
`BaseBox<T>` ([core/base.svelte.js](src/lib/core/base.svelte.js)) = `MutCoreBox` mixed with `BoxAccessorMixin`, `BoxGuardsMixin`, `BoxCommonMixin`. Accessor mixin contribute `get`/`set`/`del`; common contribute `toJSON`/`freeze`/`isFrozen`/`clone`; guard mixin contribute 14 type guards. `BaseBox` itself add `snapshot()` and `eager()`. Both `FastBox` and `Box` extend `BaseBox`.

`FastBox` ([core/fast/box.js](src/lib/core/fast/box.js)) = near-empty subclass. Exists so choice between proxy-backed and plain explicit at type level, add `const()` returning `ConstFastBox`.

`Box` ([core/proxy/box.js](src/lib/core/proxy/box.js)) override constructor to return `buildBoxProxy(this, opts)` (from [core/proxy/base.js](src/lib/core/proxy/base.js)), give transparent forwarding, callability for function values, `instanceof` propagation. `const()` return `ConstBox` (proxy-backed read-only view) sharing same machinery in read-only mode.
...
`ConstBox` ([core/proxy/const.js](src/lib/core/proxy/const.js)) extend `ConstFastBox` and wrap same proxy machinery as `Box` in read-only mode (`buildBoxProxy(this, { readOnly: true, ... })`). Forwarded reads work same as on `Box`; every write trap throws. `box.const()` return this; `fastbox.const()` return `ConstFastBox`.
...
Transparent-forwarding shape (`Boxed<T> = Box<T> & Forwarded<T>`) lives in [core/proxy/box.d.ts](src/lib/core/proxy/box.d.ts) only. No equivalent on the no-proxy side: `FastBox<T>` is the only public name. `FastBoxed<T>` was a deprecated alias removed in 0.3.0. Do not introduce a higher-kinded helper for these names, TypeScript does not support HKT and the older `BaseBoxed<T, B>` definition with `B<T>` will not type-check.
...
1. **Function target, shared at module scope.** Proxy wraps `PROXY_TARGET = function () {}` so box callable (`box()`) and constructable (`new box()`) when inner value = function or class. Same `PROXY_TARGET` used for every Box. Means **defensive traps required** to stop one Box from mutating shared target and corrupting every other Box. `defineProperty` route to self/inner. `preventExtensions` and `setPrototypeOf` throw `TypeError` with explicit messages. Do not weaken these.

2. **`isOwn` walks prototype chain, stopping at `Object.prototype`.** Svelte 5 install `$state` accessor on `Box.prototype`, not on instance, so `hasOwnProperty.call(self, 'value')` return false. Walk catch accessor, class fields, prototype methods, any subclass methods. Stopping before `Object.prototype` keep `toString`/`hasOwnProperty`/etc. forwarding to inner value. Found by running tests, fixing failures led to this design.
...
- `Boxed<T>` = `Box<T> & ForwardShape<T>` where `ForwardShape<T>` resolve to callable signature for function `T`, object surface for object `T`, `unknown` for primitives. Object and array properties forward; primitives keep only `Box<T>` surface (type narrows for safety, runtime forwarding still works).
- No `FastBoxed<T>` alias as of 0.3.0. `FastBox<T>` is the only public no-proxy name. `fastbox(...)` factory and `FastBoxedMap` / `FastBoxedSet` types resolve to `FastBox<T>` directly.
...
Tests split per module to mirror lib layout: [tests/box.svelte.test.ts](tests/box.svelte.test.ts), [tests/fastbox.svelte.test.ts](tests/fastbox.svelte.test.ts), and [tests/collections/](tests/collections/) (one file per collection type). Shared helpers in [tests/\_helpers.svelte.ts](tests/_helpers.svelte.ts); leading underscore = convention for non-test test utilities and keep file out of vitest default discovery while still letting it use runes.

Reactivity tests use `$effect.root` for setup and `flushSync()` between mutations. `withRoot` helper from `_helpers.svelte.ts` wraps root + cleanup boilerplate. Pattern:
...
1. Checks out at `fetch-depth: 0` and verify release commit ancestor of `origin/master` (defense in depth alongside `v*` tag-protection ruleset).
2. Re-runs lint, type-check, full test suite.
3. Runs `npm audit signatures` to verify every tarball in resolved dependency graph carry valid Sigstore signature from npm.
4. Verifies release tag matches `package.json` version (`v` prefix stripped).
5. Runs `prepack` (`svelte-package + publint`).
6. Generates CycloneDX SBOM (`sbom.cdx.json`) via `@cyclonedx/cdxgen -t bun` (read `bun.lock` directly), with `continue-on-error: true` so generator failure cannot break publish.
7. Publishes to npm via Trusted Publisher OIDC with provenance attestations. SemVer pre-releases (any version with hyphen, e.g. `0.2.0-rc.0`) ship under `next` dist-tag; final releases ship under `latest`.
8. Attaches SBOM to GitHub Release as downloadable asset, only if step 6 produced file.

Workflow use `id-token: write` (for OIDC) and `contents: write` (for SBOM upload). No long-lived `NPM_TOKEN` stored; npm registry exchange GitHub OIDC token for short-lived publish credential at run time.
...
- All tests run in headless Chromium through `@vitest/browser-playwright`. No Node-environment test project; everything touching `$state` need real Svelte runtime, only behave correctly in browser.
- Shared helpers go in [tests/\_helpers.svelte.ts](tests/_helpers.svelte.ts). Leading underscore = convention for non-test test utilities; file not picked up as test because name not match discovery suffix. `.svelte.ts` extension still required because body use runes.
...
- Fixtures (small classes or values used as test inputs) go inline in test file when short. Reactive classes holding `$state` fields declared at module scope, not inside `it()` callbacks: nested declarations trip Svelte `perf_avoid_nested_class` warning.
- Current suite = 91 tests across four files. Adding test should not push past ~3 seconds locally on Chromium; if it do, ask whether test exercising lib or framework.
...
Keep all six consistent. When adding public export, update README API reference, JSDoc on new symbol, this file Architecture/source-layout list, and entry under next version section in `CHANGELOG.md` in same change. When changing publish flow, security posture, or contribution policy, update `SECURITY.md` or `CONTRIBUTING.md` alongside workflow file.
...
When change affects public surface (anything exported from `@coroama/svelte-box`, runtime behavior of those exports, or types of those exports), add `CHANGELOG.md` entry under `## [Unreleased]`. Format follows [Keep a Changelog](https://keepachangelog.com): `Added`, `Changed`, `Deprecated`, `Removed`, `Fixed`, `Security`, in that order, with version-comparison links updated at bottom of file. Documentation-only changes not require CHANGELOG entry unless doc fix significant enough you want it show up on npm package page after next patch release.
