...
`FastBox` ([core/fast/box.js](src/lib/core/fast/box.js)) = near-empty subclass. Exist so choice between proxy-backed and plain explicit at type level, add `toConst()` return `ConstFastBox`.

`Box` ([core/proxy/box.js](src/lib/core/proxy/box.js)) override constructor to return `buildBoxProxy(this, opts)` (from [core/proxy/base.js](src/lib/core/proxy/base.js)), give transparent forwarding, callability for function values, `instanceof` propagation. `toConst()` return `ConstBox` (proxy-backed read-only view) share same machinery in read-only mode.
...
`ConstBox` ([core/proxy/const.js](src/lib/core/proxy/const.js)) extend `ConstFastBox` and wrap same proxy machinery as `Box` in read-only mode (`buildBoxProxy(this, { readOnly: true, ... })`). Forwarded reads work same as on `Box`; every write trap throws. `box.toConst()` return this (in borrow mode, so view stay reactive to source updates); `fastbox.toConst()` return `ConstFastBox` (also borrow mode).
