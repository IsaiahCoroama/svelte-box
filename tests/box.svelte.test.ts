...
/**
 * Helper that runs a setup function inside an effect root, returning a
 * cleanup. Effects inside `setup` will run on the next flush.
 */
function withRoot(setup: () => void): () => void {
	return $effect.root(() => {
		setup();
	});
}
...
describe('boxedMap / boxedSet: JSON.stringify', () => {
	// Documents the README claim that JSON.stringify of a boxedMap or boxedSet
	// returns "{}" because Map/Set entries are not own enumerable properties.
	// Box's `toJSON` returns `box.value` (the SvelteMap/SvelteSet), and
	// JSON.stringify of a Map or Set returns "{}".
	...
	it('Array.from / spread is the supported way to serialize collections', () => {
		const m = boxedMap<string, number>([['a', 1]]);
...
describe('Box: get / set / del helpers', () => {
	...
	it('set on a Map-wrapping box calls the inner Map.set', () => {
		const m = boxedMap<string, number>();
		// box.set is shadowed by SvelteMap.set, so this should mutate the map
		m.set('k', 1);
