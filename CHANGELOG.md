...
- **Trimmed auto-applied label set** in `.github/labeler.yml` to path-derivable labels only (`docs`, `internal`, `upgrade`). Removed `feature`, `bug`, `refactor` rules — they all mapped to `src/lib/**` and stamped every src/lib PR with all three regardless of intent. These plus `security` and `breaking` are now author/maintainer-applied. The checker's `any_of` requirement still enforces that one classification label is present.
